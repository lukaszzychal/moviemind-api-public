...
return new class extends Migration
{
    ...
    {
        Schema::create('tv_show_reports', function (Blueprint $table) {
            // Use UUID for primary key (UUIDv7 via HasUuids trait)
            $table->uuid('id')->primary();
            $table->foreignUuid('tv_show_id')->constrained('tv_shows')->cascadeOnDelete();
            $table->foreignUuid('description_id')->nullable()->constrained('tv_show_descriptions')->nullOnDelete();
    ...
    {
        Schema::dropIfExists('tv_show_reports');
    }
};
