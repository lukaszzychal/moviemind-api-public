...
final class CollectionGenericDocblockFixer implements FixStrategy
{
    private const TEMPLATE = '@property \Illuminate\Support\Collection<int, mixed> $%s';
    ...
    {
        return Str::contains($issue->message, 'has no value type specified in iterable type')
            && Str::contains($issue->message, 'Illuminate\\Support\\Collection');
    }

    public function handle(PhpstanIssue $issue, AutoFixMode $mode): ?FixSuggestion
    {
        if (! preg_match(
            '/Property\s+([A-Za-z0-9_\\\\]+)::\$([A-Za-z0-9_]+)\s+has no value type specified in iterable type\s+Illuminate\\\\Support\\\\Collection\./',
            $issue->message,
            $matches
        )) {
            return null;
        ...
        $summary = sprintf(
            'add @property Collection<int, mixed> $%s docblock for %s',
            $property,
        ...
        $preview = <<<TXT
/**
 * @property \Illuminate\Support\Collection<int, mixed> \${$property}
 */
...
TXT;
    ...
    private function injectDocblock(string $contents, PhpstanIssue $issue, string $property): ?string
    {
        ...
        $docLine = sprintf(self::TEMPLATE, $property);
