...
function get(path, options = {}) {
  ...
  return fetch(`${base}${path}`, {
    method: 'GET',
    headers: { Accept: 'application/json', 'Accept-Language': lang, ...headers },
    ...restOptions,
  })
}
...
async function parseResponse(response) {
  ...
  if (!response.ok) {
    const errorMsg = data?.message || response.statusText || `HTTP Error ${response.status}: Server returned HTML or invalid format.`
  ...
  if (text && !isJson) {
    ...
    throw new Error(`Obtained code 200, but response is not JSON. Make sure Database and API are running: ${preview}...`)
  }
...
// Global Config & Health
export async function getHealthInstance() {
  const res = await get('/health/instance')
  return parseResponse(res)
}

export async function getRootWelcome() {
